    ...
    public function run()
    {
        ...
        foreach ($users as $user) {
            $diachi[] = [
                ...
                'thanhpho' => $user->id % 2 == 0 ? 'Hà Nội' : 'Hồ Chí Minh',
                'xaphuong' => "Phường " . $user->id,
                'sonha' => "Số " . $user->id . ", Đường ABC",
        ...
        $anonymous = DB::table('nguoi_dung')
            ->where('vaitro', 'anonymous')
            ->first();

        if ($anonymous) {
            $diachi[] = [
                'ten' => $anonymous->hoten,
                'sodienthoai' => $anonymous->sodienthoai . ',0999999999',
                'thanhpho' => 'Hồ Chí Minh',
                'xaphuong' => "Phường 1",
                'sonha' => "Số 1, Đường Test",
                'diachi' => "Số 1, Đường Test, Phường 1, Q.1, Thành phố Hồ Chí Minh",
                'trangthai' => 'hoat_dong',
                'id_nguoidung' => $anonymous->id,
                'created_at' => $now,
                'updated_at' => $now,
            ];
        }
